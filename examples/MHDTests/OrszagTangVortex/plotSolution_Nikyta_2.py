...
def make_color_levels(Q,cmin,cmax,c_res=10,log_sc=True):
    if log_sc:
        ...
        for i in range(levmin,levmax):
            levels_short=[10**i]
            for j in range(1,c_res):    
                levels+=[(10*j/c_res)*10**i]
    else:
...
def make_density_plot(Q,cmin,cmax,i,j,Q_name,c_res=10,log_sc=True):   
    ...
    if log_sc:
        to_plot=ax[i][j].tricontourf(x,y,Q,levels=np.array(levels),locator=ticker.LogLocator(),cmap = 'jet')
    else:
       to_plot=ax[i][j].tricontourf(x,y,Q,levels=np.array(levels),cmap = 'jet')
...
make_density_plot(rho,np.min(rho),1.1*np.max(rho),0,1,"rho",c_res=100,log_sc=False)
make_density_plot(vv,np.min(vv),1.1*np.max(vv),0,2,"|v|",c_res=100,log_sc=False)
make_density_plot(P,np.min(P),1.1*np.max(P),1,0,"P",c_res=100,log_sc=False)
...
#make_density_plot(R0,1e-2,1,2,0,"R0")
#make_density_plot(R1,1e-2,1,2,1,"R1")
#make_density_plot(R2,1e-2,1,2,2,"R2")
make_density_plot(R0,np.quantile(R0,0.001),1.1*np.quantile(R0,0.999),2,0,"R0",c_res=100,log_sc=False)
make_density_plot(R1,np.quantile(R1,0.001),1.1*np.quantile(R1,0.999),2,1,"R1",c_res=100,log_sc=False)
make_density_plot(R2,np.quantile(R2,0.001),1.1*np.quantile(R2,0.999),2,2,"R2",c_res=100,log_sc=False)
...
ax[1][2].text(-0.45, 0.2, "R0 = h divB/B, R1 = Cos(F_mag^B), R2 = divB / |rotB|", fontsize=text_fontsize)
