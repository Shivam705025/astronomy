...
def get_positions_and_time(N_snapshots, N_part, output_dir, boxsize):
    # radius = np.zeros((N_part, N_snapshots))
...
def plot_deviation_from_circular_orbits(x, y, z, time, color, save_fig_name_suffix):
    ...
    ax2[0].plot(time, error_1, color[0], marker = "x")
    ax2[1].plot(time, error_2, color[1], marker = "x")
    ax2[1].plot(time, error_3, color[2], marker = "x")
...
def plot_deviation_from_original_data(r_1, r_2, r_3, time, color, save_fig_name_suffix):
    ...
    plt.tight_layout()
    plt.savefig("deviation_from_original_data" + save_fig_name_suffix + ".png", bbox_inches='tight')
    plt.close()

#%%Simulation with first type of units (kpc)
# N_snapshots = 201 
# N_part = 3
# output_dir = "output_1"
# boxsize = 1000.0 #kpc
...
x_1, y_1, z_1, time_1, pot_1 = get_positions_and_time(N_snapshots, N_part, output_dir, boxsize)
plot_orbits(x_1, y_1, z_1, col, save_fig_name_suffix)
r_11, r_21, r_31 = plot_deviation_from_circular_orbits(x_1, y_1, z_1, time_1, col, save_fig_name_suffix)
plot_deviation_from_original_data(r_11, r_21, r_31, time_1, col, save_fig_name_suffix)

#Second type of units (Mpc)
boxsize = 1.0 #Mpc
output_dir = "output_2"
save_fig_name_suffix = "_simulation_Mpc"
x_2, y_2, z_2, time_2, pot_2 = get_positions_and_time(N_snapshots, N_part, output_dir, boxsize)
x_2 *= 1e3 ; y_2 *= 1e3 ; z_2 *= 1e3 ; time_2 *= 1e3
plot_orbits(x_2, y_2, z_2, col, save_fig_name_suffix)
r_12, r_22, r_32 = plot_deviation_from_circular_orbits(x_2, y_2, z_2, time_2, col, save_fig_name_suffix)
plot_deviation_from_original_data(r_12, r_22, r_32, time_2, col, save_fig_name_suffix)
