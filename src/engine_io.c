...
void engine_dump_snapshot(struct engine *e) {
  ...
  clocks_gettime(&time1);
   ...
   //Shifts the particles back for writing files
  double shift_back[3] = {0.0, 0.0, 0.0};
  double print_message = 0 ; //We may not want to print anything when writting
  parser_get_opt_param_double_array(e->parameter_file, "InitialConditions:shift_back", 3,
									shift_back);
  apply_shift(e->s->parts, e->s->gparts, e->s->sinks, e->s->sparts, e->s->bparts,
			  e->s->nr_parts, e->s->nr_gparts, e->s->nr_sinks, e->s->nr_sparts,
			  e->s->nr_bparts, print_message, shift_back) ;
...
/* Dump (depending on the chosen strategy) ... */
  ...
  //Shifts the particles back to the position before writing files
  shift_back[0] = -shift_back[0] ;
  shift_back[1] = -shift_back[1] ;
  shift_back[2] = -shift_back[2] ;
  apply_shift(e->s->parts, e->s->gparts, e->s->sinks, e->s->sparts, e->s->bparts,
			  e->s->nr_parts, e->s->nr_gparts, e->s->nr_sinks, e->s->nr_sparts,
			  e->s->nr_bparts, print_message, shift_back) ;
}
